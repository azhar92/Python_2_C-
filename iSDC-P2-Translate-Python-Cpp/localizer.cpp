...
vector< vector <float> > initialize_beliefs(vector< vector <char> > grid) {
  	...
  	int width;
  	int area;
  	float beliefs_per_cell;
 
  	height = grid.size();
  	width = grid[0].size();
  	area = height * width;
  	beliefs_per_cell = 1.0/area;
  	//Initalize the grid with the belief per area
  	for (int i = 0; i<grid.size(); i++){
      	newRow.clear();
  	  	for (int j = 0; j<grid[0].size(); j++) {
          	newRow.push_back(beliefs_per_cell);
        	}
...
vector< vector <float> > move(int dy, int dx, vector < vector <float> > beliefs, float blurring) 
{
 ...
 //	return newGrid;  // for unit testing purpose
}
	...
	float p_miss) 
{
    ...
    for (int i = 0; i< height; i++ ) {
     ...
     return newGrid = normalize(newGrid);
